    ...
    def detect(self):
        ...
        if save_csv:
            with open(csv_path, 'w') as f:
                f.write("frame;people_count;looking_face_count;IDs_people;IDs_looking;bb_people;bb_looking \n")
        ...
        for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    # face visualization and add id for each face, info saved in temp_info
                    # if len_faces !=0:
                    #     temp_info = face_visualize(faces, outputs, im0, temp_info, frame_height, frame_width, frontal_face)
                    #     list_frontal_faces[frame_idx] = np.asarray(temp_info)
                    # else:
        ...
        if save_txt or save_vid or save_csv:
