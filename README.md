...
<br></br>
    if you are using anaconda environment and have problem with `pip install dlib`, try these:
<br></br>
...
If you only want to track people in crowds I recommend you to get [these weights](https://drive.google.com/file/d/1gglIwqxaH2iTvy6lZlXuAcMpd_U0GCUb/view?usp=sharing) for increased performance

```bash
python3 track.py --source 0 --yolo_weights yolov5/weights/crowdhuman_yolov5m.pt --classes 0  # class 0 for tracks persons, only
```
...
```
I've add save-csv to save result 

```bash
python3 track.py --source ... --save-csv
```
